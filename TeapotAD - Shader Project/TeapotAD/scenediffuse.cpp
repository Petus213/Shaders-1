...
void SceneDiffuse::setLightParams(QuatCamera camera)
{
	...
	//These set the light intensity for the different types of lighting
	prog.setUniform("Ds.Ld", 0.9f, 0.9f, 0.9f); //Diffuse
	prog.setUniform("As.La", 0.3f, 0.3f, 0.3f); //Ambient
	prog.setUniform("Ss.Ls", 0.3f, 0.3f, 0.3f); //Specular
...
void SceneDiffuse::render(QuatCamera camera)
{
	...
	//These set the values for Attenuation
	prog.setUniform("Ts.Ac", 2.0f); //Constant
	prog.setUniform("Ts.Al", 2.0f); //Linear
	prog.setUniform("Ts.Aq", 2.0f); //Quadratic
	...
	//The K_ elements are the reflectiveness of each type of light, and the values are set here.
	prog.setUniform("Ds.Kd", 0.51f, 1.0f, 0.49f); //Diffuse
	prog.setUniform("As.Ka", 0.51f, 1.0f, 0.49f); //Ambient
	prog.setUniform("Ss.Ks", 0.1f, 0.1f, 0.1f); //Specular

	//The Value for the shininess of Specular light is set here.
	prog.setUniform("Ss.Ns", 0.1f);
	plane->render();
	...
	//Now set up the teapot 
	 ...
	 //The K_ elements are the reflectiveness of each type of light, and the values are set here.
	 prog.setUniform("Ds.Kd", 0.46f, 0.29f, 0.0f); //Diffuse
	 prog.setUniform("As.Ka", 0.46f, 0.29f, 0.0f); //Ambient
	 prog.setUniform("Ss.Ks", 0.29f, 0.29f, 0.29f); //Specular

	 //The Value for the shininess of Specular light is set here.
	 prog.setUniform("Ss.Ns", 1.5f);
	 teapot->render(); 
